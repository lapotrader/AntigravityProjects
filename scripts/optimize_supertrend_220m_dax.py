        ...
        if in_position:
            if pos_type == 'long':
                if current_high >= tp_level:
                    pnl = ((tp_level - entry_price) * 25.0) - 6.0  # 1 punto = 25 EUR, comm. 3+3=6 EUR
                    current_equity += pnl
                    trades.append({'type': 'Long', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                elif current_close < current_supertrend:
                    pnl = ((current_close - entry_price) * 25.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Long', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                ...
                if current_low <= tp_level:
                    pnl = ((entry_price - tp_level) * 25.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Short', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                elif current_close > current_supertrend:
                    pnl = ((entry_price - current_close) * 25.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Short', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
...
def main():
    ...
    print("Inizio ottimizzazione DAX a 220m...")
    ...
    # Salva in CSV
    res_df.to_csv('dax_220m_optimization_results.csv', index=False)
    ...
    <html>
    <head>
        <title>Risultati Ottimizzazione DAX 220M</title>
        <style>
    ...
    <body>
        <h1>📊 Risultati Ottimizzazione DAX (220 Minuti)</h1>
    ...
    with open('dax_220m_optimization_report.html', 'w', encoding='utf-8') as f:
        f.write(html_template)
        
    print("\nReport HTML generato in 'dax_220m_optimization_report.html'.")
