...
def main():
    filepath = 'btp_220m.txt'
    print("Caricamento dati...")
    df = load_data(filepath)
    
    # Parametri Ottimali per 220 Minuti
    period = 20
    multiplier = 3.0
    tp_multiplier = 1.0
    ma_period = 21
    ...
    fig = make_subplots(
        ...
        row_heights=[0.7, 0.3],
        subplot_titles=(f"Prezzo BTP 220M, ST({period}, {multiplier}), SMA {ma_period} e Operazioni", "Curva di Equity (EUR)")
    )
    
    # 1. Candlestick Prezzo (mostriamo l'intero dataset dal 2023)
    fig.add_trace(
        go.Candlestick(
            x=df_result['data'],
            open=df_result['open'],
    ...
    # 3. Linea Supertrend (Rimossa come richiesto)
    ...
    fig.update_layout(
        title=dict(
            text=f'BTP 220M SUPERTREND BACKTEST - NET PROFIT: {stats["net_profit_eur"]:,.2f} €',
    ...
    # Impostiamo i rangebreaks per rimuovere i buchi dei weekend e della notte
    hide_gaps = [
        dict(bounds=["sat", "mon"]), # Nasconde i weekend
        dict(bounds=[19, 8], pattern="hour") # Nasconde la notte tra le 19:00 e le 08:00
    ]
    ...
    output_filename = 'btp_220m_stable_backtest.html'
