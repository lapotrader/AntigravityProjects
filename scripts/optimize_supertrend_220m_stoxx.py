        ...
        if in_position:
            if pos_type == 'long':
                if current_high >= tp_level:
                    pnl = ((tp_level - entry_price) * 10.0) - 6.0  # 1 punto = 10 EUR, comm. 3+3=6 EUR
                    current_equity += pnl
                    trades.append({'type': 'Long', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                elif current_close < current_supertrend:
                    pnl = ((current_close - entry_price) * 10.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Long', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                ...
                if current_low <= tp_level:
                    pnl = ((entry_price - tp_level) * 10.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Short', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
                elif current_close > current_supertrend:
                    pnl = ((entry_price - current_close) * 10.0) - 6.0
                    current_equity += pnl
                    trades.append({'type': 'Short', 'pnl': pnl, 'duration': i - entry_idx})
                    in_position = False
                    pos_type = None
...
def main():
    ...
    print("Inizio ottimizzazione Euro Stoxx a 220m...")
            ...
            for tp in tps:
                for ma_p in ma_periods:
                    ...
                    results.append({
                        ...
                        'Net Profit (EUR)': final_profit,
                        'Max Drawdown (EUR)': max_dd,
                        'Profit/DD Ratio': final_profit / max_dd if max_dd > 0 else 0
                    })
    ...
    # Salva in CSV
    res_df.to_csv('stoxx_220m_optimization_results.csv', index=False)
    
    # Genera report HTML
    html_table = res_df.head(100).to_html(index=False, float_format=lambda x: f"{x:,.2f}", classes='data')
    
    html_template = f"""
    <html>
    <head>
        <title>Risultati Ottimizzazione Euro Stoxx 50 220M</title>
        <style>
    ...
    <body>
        <h1>📊 Risultati Ottimizzazione Euro Stoxx 50 (220 Minuti)</h1>
        <p>Analisi iterando su: Supertrend Period, Supertrend Multiplier, SMA, e Take Profit.</p>
    ...
    with open('stoxx_220m_optimization_report.html', 'w', encoding='utf-8') as f:
        f.write(html_template)
        
    print("\nReport HTML generato in 'stoxx_220m_optimization_report.html'.")
