...
mcp = FastMCP("Trading Console T3")
...
def _fetch_bars(data_da):
    ...
    if not lines or not lines[0].startswith("outcome=OK"):
        return None, lines[0] if lines else "empty response"
    bars = []
    for line in lines[1:]:
        line = line.strip()
...
@mcp.tool()
def t3_get_quote() -> str:
    """Ottieni la quotazione live del DAX Future da T3OPEN (last, bid, ask, variazione %, volume)"""
    return json.dumps(_fetch_quote(), indent=2)


@mcp.tool()
def t3_get_recent_bars(count: int = 20) -> str:
    """Leggi le ultime N barre dal file dati locale (DAX 3min)"""
    try:
...
def t3_fetch_new_bars() -> str:
    ...
    last_new = new[-1].split("\t")[0]
    return f"Scaricate {len(new)} nuove barre (-> {last_new})"
...
def semaforo_signal() -> str:
    ...
    if result.get("semaforo_verde"):
        ...
        if result.get("fascia_aperta"):
            reasons.append("fascia OK")
        else:
            reasons.append("fuori fascia")
        if not result.get("posizione_aperta"):
            reasons.append("posizione chiusa")
        else:
...
def read_data_file(filename: str) -> str:
    """Legge le prime righe di un file dalla cartella dati/"""
...
def list_scripts() -> str:
    """Elenca gli script Python disponibili nella cartella listati/"""
    scripts = [f.stem for f in sorted(LISTATI.glob("*.py")) if f.stem != "trading_mcp_server"]
    return "\n".join(scripts)


@mcp.tool()
def read_report() -> str:
    """Legge il report OOS validation (HTML)"""
    html_files = list(REPORTS.glob("*.html"))
    if not html_files:
        return "Nessun report HTML trovato."
...
def run_script(script_name: str, args: str = "") -> str:
    """Esegue uno script Python dalla cartella listati/ e restituisce l'output"""
...
@mcp.tool()
def trade_history(count: int = 10) -> str:
    """Mostra gli ultimi N trade simulati dal log del semaforo"""
    log_path = BASE / "trade_log.txt"
    if not log_path.exists():
        return "Nessun trade log trovato. Avvia consolet3.py per generare log."
    try:
